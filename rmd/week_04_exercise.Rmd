...
```{r}
# Import data
rabies_raw <- read_csv(here("data/rabies_dataset.csv"))
```
...
```{r}
# Clean data
rabies_clean <- rabies_raw %>%
  select(Respondent_ID, everything()) %>%  # Bring Respondent_ID column to left of data frame
  select(!Education)  # Remove Education column from data frame
```
...
```{r}
# Subset the data
rabies_demog <- rabies_clean %>% 
  select(Age, Gender, Background, contains("Vaccination"))  # Select demographic and vaccination columns

# Export the data
write_csv(rabies_demog, file = here("outputs/rabies_demog_data.csv"))
```
...
```{r}
# Subset the data
eval_questions <- c("DogBite",
                    "DogSpread",
                    "Transmission",
                    "PreventableVaccination",
                    "Signs",
                    "Doctor",
                    "PreferVaccination")

rabies_knowledge_eval <- rabies_clean %>% 
  select(eval_questions)  # Select columns with knowledge evaluation questions

# Export the data
write_csv(rabies_knowledge_eval, file = here("outputs/rabies_knowledge_eval_data.csv"))
```
...
```{r}
# Subset the data
rabies_male_over_18 <- rabies_clean %>% 
  filter(Gender == 1 & Age > 1)  # Filter rows for males over age 18

# Export the data
write_csv(rabies_male_over_18, file = here("outputs/rabies_male_over_18_data.csv"))
```
...
```{r}
# Subset the data
rabies_at_risk_respondents <- rabies_clean %>% 
  filter(Domestication == 1 |
           Facilities == 2 |
           AffordableVaccination == 2)  # Filter rows for "at-risk" individuals

# Export the data
write_csv(rabies_at_risk_respondents, file = here("outputs/rabies_at_risk_respondents_data.csv"))
```
...
```{r}
# Subset the data
rabies_ideal_respondents <- rabies_clean %>% 
  filter(DogBite == 1 &
           Signs == 1 &
           Doctor == 1)  # Filter rows for respondents with ideal KAPs

# Export the data
write_csv(rabies_ideal_respondents, file = here("outputs/rabies_ideal_respondents_data.csv"))
```
...
# Create table of contents for Excel workbook
table_of_contents <- data.frame(
  Sheet_Name = c("Rabies Demog",
                 "Rabies Knowledge Eval",
                 "Rabies Male Over 18",
                 "Rabies At Risk Respondents",
                 "Rabies Ideal Respondents"),
  Description = c("This sheet contains a data subset with demographic and vaccination information",
                  "This sheet contains a data subset with responses to knowledge evaluation questions",
                  "This sheet contains a data subset for male respondents aged over 18 years",
                  "This sheet contains a data subset for 'at-risk' respondents",
                  "This sheet contains a data subset for respondents with ideal KAPs")
  )

# Set Excel workbook header style
rabies_header_style <- createStyle(
  textDecoration = "BOLD", fontColour = "#FFFFFF", fgFill = "#4F80BD"
  )

# Assign data subsets to sheets in Excel workbook
rabies_data_subsets <- list("TOC" = table_of_contents,
                            "Rabies Demog" = rabies_demog,
                            "Rabies Knowledge Eval" = rabies_knowledge_eval,
                            "Rabies Male Over 18" = rabies_male_over_18,
                            "Rabies At Risk Respondents" = rabies_at_risk_respondents,
                            "Rabies Ideal Respondents" = rabies_ideal_respondents
                            )

# Create Excel workbook
write.xlsx(x = rabies_data_subsets,
           file = here("outputs/rabies_data_subsets.xlsx"),
           headerStyle = rabies_header_style
           )
